...
function concat(str1, str2) {
  let newSelection = str1.slice(0, 2) + str2.slice(-3);
  let toUpperCaseString = newSelection.toUpperCase();
  return toUpperCaseString;
}
console.log(concat("ciao", "andrei")); */
...
function RandNumb() {
  for (let i = 0; i < 10; i++) {
    ...
    Random10Numbers.push(randomnumb);
  }
}
RandNumb();
console.log(Random10Numbers); */
...
const evenNumbers = onlyNumbers.filter((evenNumber) => evenNumber % 2 === 0);
...
onlyNumbers.forEach((number) => (sum += number));
...
const MapEpicode = Epicode.map((str) => str.length);

console.log(MapEpicode);
 */
...
for (let i = 0; i < 100; i++) {
  if (i % 2 !== 0) {
    ...
    i % 2 === 0;
    console.log("Numeri non dispari");
  }
}
...
console.log(FilmPiuVecchio.Title);
/* ESERCIZIO 10
...
const FilmThisEra = movies.filter((movie) => movie.Year >= 2000);

console.log(FilmThisEra);
